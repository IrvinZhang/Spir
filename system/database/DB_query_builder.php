...
abstract class SP_DB_query_builder extends SP_DB_driver {
	...
	protected function _compile_wh($qb_key)
	{
		if (count($this->$qb_key) > 0)
		{
			for ($i = 0, $c = count($this->$qb_key); $i < $c; $i++)
			{
				...
				for ($sp = 0, $cc = count($conditions); $sp < $cc; $sp++)
				{
					if (($op = $this->_get_operator($conditions[$sp])) === FALSE
						OR ! preg_match('/^(\(?)(.*)('.preg_quote($op, '/').')\s*(.*(?<!\)))?(\)?)$/i', $conditions[$sp], $matches))
					{
						continue;
					}
					...
					$conditions[$sp] = $matches[1].$this->protect_identifiers(trim($matches[2]))
