 ...
 */
class SP_DB_pdo_sqlsrv_driver extends SP_DB_pdo_driver {
	...
	protected function _delete($table)
	{
		if ($this->qb_limit)
		{
			return 'WITH sp_delete AS (SELECT TOP '.$this->qb_limit.' * FROM '.$table.$this->_compile_wh('qb_where').') DELETE FROM sp_delete';
		}
	...
	protected function _limit($sql)
	{
		...
		if ($this->qb_offset && ! empty($this->qb_orderby))
		{
			...
			// Get the fields to select from our subquery, so that we can avoid SP_rownum appearing in the actual results
			...
			return 'SELECT '.$select." FROM (\n\n"
				.preg_replace('/^(SELECT( DISTINCT)?)/i', '\\1 ROW_NUMBER() OVER('.trim($orderby).') AS '.$this->escape_identifiers('SP_rownum').', ', $sql)
				."\n\n) ".$this->escape_identifiers('SP_subquery')
				."\nWHERE ".$this->escape_identifiers('SP_rownum').' BETWEEN '.($this->qb_offset + 1).' AND '.$limit;
		}
