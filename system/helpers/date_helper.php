...
if ( ! function_exists('timespan'))
{
	...
	function timespan($seconds = 1, $time = '', $units = 7)
	{
		...
		if ($years > 0)
		{
			$str[] = $years.' '.$SP->lang->line($years > 1 ? 'date_years' : 'date_year');
		}
		...
		if (count($str) < $units && ($years > 0 OR $months > 0))
		{
			if ($months > 0)
			{
				$str[] = $months.' '.$SP->lang->line($months > 1 ? 'date_months' : 'date_month');
			}
		...
		if (count($str) < $units && ($years > 0 OR $months > 0 OR $weeks > 0))
		{
			if ($weeks > 0)
			{
				$str[] = $weeks.' '.$SP->lang->line($weeks > 1 ? 'date_weeks' : 'date_week');
			}
		...
		if (count($str) < $units && ($months > 0 OR $weeks > 0 OR $days > 0))
		{
			if ($days > 0)
			{
				$str[] = $days.' '.$SP->lang->line($days > 1 ? 'date_days' : 'date_day');
			}
		...
		if (count($str) < $units && ($days > 0 OR $hours > 0))
		{
			if ($hours > 0)
			{
				$str[] = $hours.' '.$SP->lang->line($hours > 1 ? 'date_hours' : 'date_hour');
			}
		...
		if (count($str) < $units && ($days > 0 OR $hours > 0 OR $minutes > 0))
		{
			if ($minutes > 0)
			{
				$str[] = $minutes.' '.$SP->lang->line($minutes > 1 ? 'date_minutes' : 'date_minute');
			}
		...
		if (count($str) === 0)
		{
			$str[] = $seconds.' '.$SP->lang->line($seconds > 1 ? 'date_seconds' : 'date_second');
		}
...
if ( ! function_exists('timezone_menu'))
{
	...
	function timezone_menu($default = 'UTC', $class = '', $name = 'timezones', $attributes = '')
	{
		...
		foreach (timezones() as $key => $val)
		{
			...
			$menu .= '<option value="'.$key.'"'.$selected.'>'.$SP->lang->line($key)."</option>\n";
		}
