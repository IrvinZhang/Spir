...
class SP_Session {
	...
	public function __construct(array $params = array())
	{
		...
		$class = $this->_sp_load_classes($this->_driver);
		...
		{
			if ( ! isset($_SESSION['__sp_last_regenerate']))
			{
				$_SESSION['__sp_last_regenerate'] = time();
			}
			elseif ($_SESSION['__sp_last_regenerate'] < (time() - $regenerate_time))
			{
	 ...
	 * classes. It kind of emulates the SP_Driver library, but is
	 * self-sufficient.
	 *
	 * @param	string	$driver	Driver name
	 * @return	string	Driver class name
	 */
	protected function _sp_load_classes($driver)
	{
		...
		if ( ! class_exists('SP_Session_driver', FALSE))
		{
			require_once(
		...
		// Allow custom drivers without the SP_ or MY_ prefix
		...
		if ( ! class_exists('SP_'.$class, FALSE))
		{
			...
			if ( ! class_exists('SP_'.$class, FALSE) && ! class_exists($class, FALSE))
			{
	 ...
	 * @return	void
	 */
	protected function _sp_init_vars()
	{
		if ( ! empty($_SESSION['__sp_vars']))
		{
			$current_time = time();

			foreach ($_SESSION['__sp_vars'] as $key => &$value)
			{
				if ($value === 'new')
				{
					$_SESSION['__sp_vars'][$key] = 'old';
				}
				// Hacky, but 'old' will (implicitly) always be less than time() ;)
				// DO NOT move this above the 'new' check!
				elseif ($value < $current_time)
				{
					unset($_SESSION[$key], $_SESSION['__sp_vars'][$key]);
				}
			}

			if (empty($_SESSION['__sp_vars']))
			{
				unset($_SESSION['__sp_vars']);
			}
		}
	...
	public function mark_as_flash($key)
	{
		if (is_array($key))
		{
			...
			$_SESSION['__sp_vars'] = isset($_SESSION['__sp_vars'])
				? array_merge($_SESSION['__sp_vars'], $new)
				: $new;
	...
	{
		if ( ! isset($_SESSION['__sp_vars']))
		{
			return array();
		}

		$keys = array();
		foreach (array_keys($_SESSION['__sp_vars']) as $key)
		{
			is_int($_SESSION['__sp_vars'][$key]) OR $keys[] = $key;
		}
	...
	public function unmark_flash($key)
	{
		...
		foreach ($key as $k)
		{
			if (isset($_SESSION['__sp_vars'][$k]) && ! is_int($_SESSION['__sp_vars'][$k]))
			{
				unset($_SESSION['__sp_vars'][$k]);
			}
		}
	...
	public function mark_as_temp($key, $ttl = 300)
	{
		...
		if (is_array($key))
		{
			...
			$_SESSION['__sp_vars'] = isset($_SESSION['__sp_vars'])
				? array_merge($_SESSION['__sp_vars'], $temp)
				: $temp;
	...
	public function get_temp_keys()
	{
		...
		foreach (array_keys($_SESSION['__sp_vars']) as $key)
		{
			is_int($_SESSION['__sp_vars'][$key]) && $keys[] = $key;
		}
	 ...
	 * Session destroy
	 *
	 * Legacy SP_Session compatibility method
	 *
	 * @return	void
	 */
	...
	public function flashdata($key = NULL)
	{
		if (isset($key))
		{
			return (isset($_SESSION['__sp_vars'], $_SESSION['__sp_vars'][$key], $_SESSION[$key]) && ! is_int($_SESSION['__sp_vars'][$key]))
				? $_SESSION[$key]
				: NULL;
		}
