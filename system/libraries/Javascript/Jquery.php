...
class SP_Jquery extends SP_Javascript {
	...
	protected function _updater($container = 'this', $controller = '', $options = '')
	{
		...
		$controller = (strpos('://', $controller) === FALSE) ? $controller : $this->SP->config->site_url($controller);

		// ajaxStart and ajaxStop are better choices here... but this is a stop gap
		if ($this->SP->config->item('javascript_ajax_img') === '')
		{
			$loading_notifier = 'Loading...';
		}
		else
		{
			$loading_notifier = '<img src="'.$this->SP->config->slash_item('base_url').$this->SP->config->item('javascript_ajax_img').'" alt="Loading" />';
		}
	...
	protected function _compile($view_var = 'script_foot', $script_tags = TRUE)
	{
		...
		$this->SP->load->vars(array('library_src' => $external_scripts));
		...
		$this->SP->load->vars(array($view_var => $output));
	}
