...
class SP_Unit_test {
	...
	public function report($result = array())
	{
		...
		foreach ($result as $res)
		{
			...
			{
				if ($key === $SP->lang->line('ut_result'))
				{
					if ($val === $SP->lang->line('ut_passed'))
					{
						$val = '<span style="color: #0C0;">'.$val.'</span>';
					}
					elseif ($val === $SP->lang->line('ut_failed'))
					{
	...
	public function result($results = array())
	{
		...
		foreach ($results as $result)
		{
			...
			foreach ($result as $key => $val)
			{
				...
				{
					if (FALSE !== ($line = $SP->lang->line(strtolower('ut_'.$val), FALSE)))
					{
						$val = $line;
					}
				}

				$temp[$SP->lang->line('ut_'.$key, FALSE)] = $val;
			}
