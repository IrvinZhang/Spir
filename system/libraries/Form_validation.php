...
class SP_Form_validation {

	/**
	 * Reference to the Spir instance
	 *
	 * @var object
	 */
	protected $SP;
	 ...
	 * @param	mixed	$rules
	 * @param	array	$errors
	 * @return	SP_Form_validation
	 */
	public function set_rules($field, $label = '', $rules = array(), $errors = array())
	{
		// No reason to set rules if we have no POST data
		// or a validation array has not been specified
		if ($this->SP->input->method() !== 'post' && empty($this->validation_data))
		{
			return $this;
		}
	 ...
	 * each array due to the limitations of SP's singleton
	 *
	 * @param	array	$data
	 * @return	SP_Form_validation
	 */
	...
	public function run($group = '')
	{
		...
		if (count($this->_field_data) === 0)
		{
			...
			if (empty($group))
			{
				...
				$group = trim($this->SP->uri->ruri_string(), '/');
				isset($this->_config_rules[$group]) OR $group = $this->SP->router->class.'/'.$this->SP->router->method;
			}
		...
		$this->SP->lang->load('form_validation');
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		foreach ($rules as $rule)
		{
			...
			if ($callback OR $callable !== FALSE)
			{
				if ($callback)
				{
					...
					else
					{
						...
						$result = $this->SP->$rule($postdata, $param);
					}
				}
				else
				{
			...
			if ($result === FALSE)
			{
				...
				if ( ! is_string($rule))
				{
					$line = $this->SP->lang->line('form_validation_error_message_not_set').'(Anonymous function)';
				}
				else
				{
	...
	protected function _get_error_message($rule, $field)
	{
		...
		elseif (FALSE !== ($line = $this->SP->lang->line('form_validation_'.$rule)))
		{
			return $line;
		}
		// DEPRECATED support for non-prefixed keys, lang file again
		elseif (FALSE !== ($line = $this->SP->lang->line($rule, FALSE)))
		{
			return $line;
		}

		return $this->SP->lang->line('form_validation_error_message_not_set').'('.$rule.')';
	}
	...
	protected function _translate_fieldname($fieldname)
	{
		...
		if (sscanf($fieldname, 'lang:%s', $line) === 1 && FALSE === ($fieldname = $this->SP->lang->line($line, FALSE)))
		{
			return $line;
		}
	...
	public function is_unique($str, $field)
	{
		...
		return isset($this->SP->db)
			? ($this->SP->db->limit(1)->get_where($table, array($field => $str))->num_rows() === 0)
			: FALSE;
	}
	...
	{
		return $this->SP->input->valid_ip($ip, $which);
	}
	...
	{
		return $this->SP->security->strip_image_tags($str);
	}
	 ...
	 * results of any previous validation routine due to the SP singleton.
	 *
	 * @return	SP_Form_validation
	 */
