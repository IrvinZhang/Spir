...
class SP_Xmlrpc {
	 ...
	 * @var	string
	 */
	public $xmlrpcName		= 'XML-RPC for Spir';
 ...
 * @link		https://Spir.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Client extends SP_Xmlrpc
{
	/**
	 * Path
	 *
	...
	public function decode($array = NULL)
	{
		...
		if (is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = $SP->security->xss_clean($array[$key]);
				}
			}
		...
		elseif ($this->xss_clean)
		{
			$result = $SP->security->xss_clean($result);
		}
 ...
 * @link		https://Spir.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Message extends SP_Xmlrpc
{
	...
	public function output_parameters(array $array = array())
	{
		...
		for ($i = 0, $c = count($this->params); $i < $c; $i++)
		{
			...
			{
				$parameters[] = ($this->xss_clean) ? $SP->security->xss_clean($a_param) : $a_param;
			}
		}
 ...
 * @link		https://Spir.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Values extends SP_Xmlrpc
{
	/**
	 * Value data
	 *
