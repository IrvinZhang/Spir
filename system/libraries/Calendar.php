...
class SP_Calendar {
	...
	public function initialize($config = array())
	{
		...
		{
			$this->next_prev_url = $this->SP->config->site_url($this->SP->router->class.'/'.$this->SP->router->method);
		}
	...
	public function get_month_name($month)
	{
		...
		return ($this->SP->lang->line($month_names[$month]) === FALSE)
			? ucfirst(substr($month_names[$month], 4))
			: $this->SP->lang->line($month_names[$month]);
	}
	...
	public function get_day_names($day_type = '')
	{
		...
		{
			$days[] = ($this->SP->lang->line('cal_'.$day_names[$i]) === FALSE) ? ucfirst($day_names[$i]) : $this->SP->lang->line('cal_'.$day_names[$i]);
		}
